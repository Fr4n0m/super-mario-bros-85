...
function preload() {
  ...
  this.load.spritesheet(
    "goomba",
    "assets/entities/" + levelStyle + "/goomba.png",
  ...
  this.load.spritesheet("shell", "assets/entities/shell.png", {
    frameWidth: 16,
    frameHeight: 15,
  });

  this.load.spritesheet("fireball", "assets/entities/fireball.png", {
    frameWidth: 8,
    frameHeight: 8,
  });
  this.load.spritesheet(
  ...
  this.load.image(
    ...
    "assets/scenery/horizontal-final-tube.png"
  );
  this.load.image(
    "vertical-extralarge-tube",
    "assets/scenery/vertical-large-tube.png"
  );
  this.load.image(
    "vertical-small-tube",
  ...
  this.load.image(
    ...
    "assets/scenery/vertical-medium-tube.png"
  );
  this.load.image(
    "vertical-large-tube",
    "assets/scenery/vertical-large-tube.png"
  ...
  this.load.image(
    "floorbricks",
    "assets/scenery/" + levelStyle + "/floorbricks.png"
  );
  this.load.image(
    "start-floorbricks",
  ...
  this.load.image(
    "emptyBlock",
    "assets/blocks/" + levelStyle + "/emptyBlock.png"
  );
  this.load.image(
    "immovableBlock",
    "assets/blocks/" + levelStyle + "/immovableBlock.png"
  );
  this.load.spritesheet(
    "brick-debris",
  ...
  this.load.spritesheet(
    "mistery-block",
    "assets/blocks/" + levelStyle + "/misteryBlock.png",
    { frameWidth: 16, frameHeight: 16 }
  );
  this.load.spritesheet(
    "custom-block",
  ...
  this.load.audio(
    "underground-music",
    "assets/sound/music/underground/theme.mp3"
  );
  this.load.audio(
    "hurry-up-music",
    "assets/sound/music/" + levelStyle + "/hurry-up-theme.mp3"
  );
  ...
  this.load.audio(
    "powerup-appears",
    "assets/sound/effects/powerup-appears.mp3"
  );
  this.load.audio(
    "consume-powerup",
    "assets/sound/effects/consume-powerup.mp3"
  );
...
function initSounds() {
  ...
  this.undergroundMusicTheme = this.sound.add("underground-music", {
    volume: 0.15,
  });
  ...
  for (i = 0; i <= platformPieces; i++) {
    ...
    if (
      pieceStart >=
        (lastWasHole > 0 ||
          lastWasStructure > 0 ||
          worldWidth - platformPiecesWidth * 4) ||
      number <= 0 ||
      ...
      this.fallProtectionGroup.add(
        this.add
          .rectangle(pieceStart, screenHeight - platformHeight, 5, 5)
          .setOrigin(1, 1)
      );
    }
    pieceStart += platformPiecesWidth * 2;
...
function drawStartScreen() {
  ...
  // Draw sky
  this.add
    .rectangle(0, 0, screenWidth, screenHeight, 0x8585ff)
    .setOrigin(0).depth = -1;
  ...
  this.add
    .image(screenWidth / 50, screenHeight / 3, "cloud1")
    .setScale(screenHeight / 1725);
  this.add
    .image(screenWidth / 1.25, screenHeight / 2, "cloud1")
    .setScale(screenHeight / 1725);
  this.add
    .image(screenWidth / 1.05, screenHeight / 6.5, "cloud2")
    .setScale(screenHeight / 1725);
  this.add
    .image(screenWidth / 3, screenHeight / 3.5, "cloud2")
    .setScale(screenHeight / 1725);
  this.add
    .image(screenWidth / 2.65, screenHeight / 2.8, "cloud2")
    .setScale(screenHeight / 1725);
  ...
  this.add
    .image(screenWidth / 50, propsY, "mountain2")
    .setOrigin(0, 1)
    .setScale(screenHeight / 517);
  this.add
    .image(screenWidth / 300, propsY, "mountain1")
    .setOrigin(0, 1)
    .setScale(screenHeight / 517);

  this.add
    .image(screenWidth / 4, propsY, "bush1")
    .setOrigin(0, 1)
    .setScale(screenHeight / 609);
  this.add
    .image(screenWidth / 1.55, propsY, "bush2")
    .setOrigin(0, 1)
    .setScale(screenHeight / 609);
  this.add
    .image(screenWidth / 1.5, propsY, "bush2")
    .setOrigin(0, 1)
    .setScale(screenHeight / 609);

  this.add
    .tileSprite(screenWidth / 15, propsY, 350, 35, "fence")
    .setOrigin(0, 1)
    .setScale(screenHeight / 863);
  ...
  this.add
    .image(
      screenCenterX * 1.12,
      screenHeight - platformHeight * 1.5,
      "settings-bubble"
    )
...
function update(delta) {
  ...
  if (
    ...
    furthestPlayerPos = player.x;
    const worldBounds = this.physics.world.setBounds(
      camera.worldView.x,
      0,
      worldWidth,
      screenHeight
