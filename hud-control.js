function createHUD() {
  ...
  this.highScoreText = this.add
    .text(screenWidth / 2, posY, "HIGH SCORE\n 000000", {
      fontFamily: "pixel_nums",
      fontSize: screenWidth / 65,
      align: "center",
    })
  ...
  this.timeLeftText = this.add.text(
    ...
    { fontFamily: "pixel_nums", fontSize: screenWidth / 65, align: "right" }
  );
...
function addToScore(num, originObject) {
  ...
  const textEffect = this.add.text(
    originObject.getBounds().x,
    originObject.getBounds().y,
    num,
    {
      fontFamily: "pixel_nums",
...
function gameOverScreen(outOfTime = false) {
  ...
  let gameOverScreen = this.add
    .rectangle(0, screenHeight / 2, worldWidth, screenHeight, 0x000000)
    .setScrollFactor(0);
