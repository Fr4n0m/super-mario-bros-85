...
function revealHiddenBlock(player, block) {
  ...
  if (random < 90) {
    ...
    this.powerUpAppearsSound.play();
    let fireFlower = this.physics.add
...
function drawDestroyedBlockParticles(block) {
  ...
  for (let particleCoords of particles) {
    let particle = this.physics.add
      .sprite(particleCoords[0], particleCoords[1], "brick-debris")
    ...
    particle.setVelocityX(
      particleCoords[0] === playerBounds.left
        ? -(screenWidth / 25.6)
        : screenWidth / 25.6
    );
